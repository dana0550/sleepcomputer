...
final class MenuBarControllerTests: XCTestCase {
    ...
    func testLockOnLidCloseTriggersOncePerCloseCycleWhenAllGatesAreEnabled() async {
        ...
        lidMonitorMock.emit(true)
        await waitForCondition { lockMock.lockCalls == 1 }
    ...
    func testQueuedLidCloseDoesNotLockAfterGateTurnsOff() async {
        let store = AppStateStore(userDefaults: UserDefaults(suiteName: "MenuBarControllerTests.\(UUID().uuidString)")!)
        let openMock = OpenLidMock()
        let closedMock = ClosedLidMock()
        let setupMock = ClosedLidSetupMock()
        ...
        lidMonitorMock.emitQueued(true, delayNanoseconds: 50_000_000)
        controller.requestFullAwakeChange(false)
    ...
    func testLockOnLidCloseDoesNotRunParallelLockAttempts() async {
        let store = AppStateStore(userDefaults: UserDefaults(suiteName: "MenuBarControllerTests.\(UUID().uuidString)")!)
        let openMock = OpenLidMock()
        let closedMock = ClosedLidMock()
        let setupMock = ClosedLidSetupMock()
        ...
        controller.setLockOnLidCloseEnabled(true)
        await controller.setFullAwakeEnabled(true)
        lockMock.suspendNextLock = true

        lidMonitorMock.emit(true)
        await waitForCondition { lockMock.isWaitingToResumeLock }

        lidMonitorMock.emit(false)
        lidMonitorMock.emit(true)
        try? await Task.sleep(nanoseconds: 60_000_000)

        XCTAssertEqual(lockMock.lockCalls, 1)
        XCTAssertEqual(lockMock.maxConcurrentLockCalls, 1)

        lockMock.resumePendingLockIfNeeded()
        await waitForCondition { !lockMock.isWaitingToResumeLock }
    }

    func testLockMonitoringStopsImmediatelyWhenTurningFullAwakeOff() async {
        let store = AppStateStore(userDefaults: UserDefaults(suiteName: "MenuBarControllerTests.\(UUID().uuidString)")!)
        let openMock = OpenLidMock()
        let closedMock = ClosedLidMock()
        let setupMock = ClosedLidSetupMock()
...
private final class LidStateMonitorMock: LidStateMonitoring {
    ...
    func emitQueued(_ isClosed: Bool, delayNanoseconds: UInt64 = 0) {
        guard let callback = onLidStateChange else {
            return
        }
...
private final class ComputerLockMock: ComputerLockControlling {
    ...
    private(set) var maxConcurrentLockCalls = 0
    private var concurrentLockCalls = 0
    private var pendingLockContinuation: CheckedContinuation<Void, Never>?

    var isWaitingToResumeLock: Bool {
        pendingLockContinuation != nil
    }

    func lockNow() async throws {
        lockCalls += 1
        concurrentLockCalls += 1
        maxConcurrentLockCalls = max(maxConcurrentLockCalls, concurrentLockCalls)
        defer {
            concurrentLockCalls -= 1
        }
        ...
        if suspendNextLock {
            ...
            await withCheckedContinuation { continuation in
                pendingLockContinuation = continuation
            }
        }
    ...
    func resumePendingLockIfNeeded() {
        pendingLockContinuation?.resume()
        pendingLockContinuation = nil
    }
}
