...
- `Lock Computer on Lid Close`: optional lock behavior during lid-close events while `Full Awake` is ON (shown on clamshell Macs). Lock now requires a verifiable lock command path and confirmed locked-session state; when unavailable on a macOS build, the setting remains visible but disabled with an inline reason.
